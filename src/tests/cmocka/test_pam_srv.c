...
static void passkey_test_done(struct tevent_req *req)
{
    ...
    struct passkey_auth_data *data;

    data = talloc_zero(pam_test_ctx, struct passkey_auth_data);
    pam_passkey_auth_recv(req, pam_test_ctx, data, &pam_test_ctx->child_status);
    talloc_zfree(req);

    /* Overwrite the child status to successful, as we are faking authentication
     * with sample passkey data */
...
void test_pam_passkey_preauth_no_passkey(void **state)
{
    ...
    mock_input_pam_passkey(pam_test_ctx, "pamuser", NULL, NULL, "1234",
                                         NULL, NULL, NULL);
...
void test_pam_passkey_preauth_found(void **state)
{
    ...
    mock_input_pam_passkey(pam_test_ctx, "pamuser", NULL, NULL, "1234", NULL,
...
void test_pam_passkey_auth_send(void **state)
{
    ...
    pk_data->public_key = talloc_strdup(tmp_ctx, SSSD_TEST_PASSKEY_PK);
    pk_data->key_handle = talloc_strdup(tmp_ctx, SSSD_TEST_PASSKEY_KEY_HANDLE);
    ...
    req = pam_passkey_auth_send(tmp_ctx, pam_test_ctx->tctx->ev,
                                ...
                                pk_data, pam_test_ctx->tctx->dom->name);
    assert_non_null(req);
