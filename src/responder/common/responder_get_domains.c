...
static void get_subdomains_done(struct tevent_req *subreq)
{
    ...
    sbus_call_dp_dp_getDomains_recv(subreq, &state->dp_error);
    talloc_zfree(subreq);
...
sss_dp_get_domains_process(struct tevent_req *subreq)
{
    ...
    ret = get_subdomains_recv(subreq, subreq, &dp_err);
    talloc_zfree(subreq);
    if (ret != EOK) {
        goto fail;
    }
...
static void sss_dp_get_account_domain_done(struct tevent_req *subreq)
{
    ...
    if (state->dp_error != ERR_OK) {
        DEBUG(state->dp_error == ERR_GET_ACCT_DOM_NOT_SUPPORTED ? SSSDBG_TRACE_INTERNAL
              ...
              (unsigned int)state->dp_error, (unsigned int)state->dp_error,
              sss_strerror(state->dp_error));
