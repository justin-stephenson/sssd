...
void passkey_kerberos_auth_cb(struct tevent_req *req)
{
    ...
    ret = pam_passkey_auth_recv(req, &child_status, &buf, &buf_len);
    talloc_free(req);
    if (ret != EOK) {
...
int passkey_child_execute(TALLOC_CTX *mem_ctx,
                           struct cli_ctx *cli_ctx,
                           struct tevent_context *ev,
                           struct pam_auth_req *pam_req,
                           struct pam_ctx *pam_ctx,
                           struct pam_data *pd,
                           enum passkey_child_op op);
void passkey_child_execute_done(struct tevent_req *req);
                               ...
                               enum passkey_child_op op)
{
    ...
    tevent_req_set_callback(req, passkey_child_execute_done, pk_check_ctx);
done:
...
void passkey_preflight_done(struct tevent_req *req)
{
    ...
    if (preflight_ctx->pd->cmd == SSS_PAM_PREAUTH) {
        /* FIXME, use actual data from buf (JSON parsed?) */
        /* FIXME: Update SSS_PAM_PASSKEY_INFO help descrption */
        uint32_t pin_attempts = 3;

        ret = pam_add_response(preflight_ctx->pd, SSS_PAM_PASSKEY_INFO, sizeof(uint32_t),
                               (const uint8_t *) &pin_attempts);
        if (ret != EOK) {
...
static errno_t passkey_child_exec(struct tevent_req *req)
{
    ...
    if (child_pid == 0) { /* child */
        ...
        /* kerberos auth and preflight operations read output from passkey child, will
         * end up in pam_passkey_child_read_data() */
        if (state->op == PAM_PASSKEY_OP_KERBEROS_AUTH || state->op == PAM_PASSKEY_OP_PREFLIGHT) {
