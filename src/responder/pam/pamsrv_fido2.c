/*
   ...
   PAM Responder - fido2 related requests
...
struct pam_fido2_auth_send_state {
    struct pam_data *pd;
...
errno_t parse_fido2_verify_opts(TALLOC_CTX *mem_ctx,
                                const char *verify_opts,
                                enum user_verification *_user_verification);

static errno_t fido2_child_exec(struct tevent_req *req);
static void pam_fido2_auth_done(int child_status,
                                struct tevent_signal *sige,
                                void *pvt);

struct tevent_req *
pam_fido2_auth_send(TALLOC_CTX *mem_ctx,
                    struct tevent_context *ev,
                    int timeout,
                    bool debug_libfido2,
                    const char *verify_opts,
                    struct pam_data *pd)
{
    ...
    struct pam_fido2_auth_send_state *state;
    size_t arg_c = 0;
    int num_args;
    int ret;
    enum user_verification verify_setting = UNSET;

    req = tevent_req_create(mem_ctx, &state, struct pam_fido2_auth_send_state);
    if (req == NULL) {
        return NULL;
    }
    ...
    state->logfile = FIDO2_CHILD_LOG_FILE;
    ...
    ret = parse_fido2_verify_opts(state, verify_opts, &verify_setting);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE, "Unable to parse fido2 verificaton options.\n");
        goto done;
    }
    ...
    switch (verify_setting) {
        ...
        case OFF:
            ...
            break;
        /* FIDO2 helper will assume "default" */
        case UNSET:
        default:
    }
...
static errno_t fido2_child_exec(struct tevent_req *req)
{
    ...
    state = tevent_req_data(req, struct pam_fido2_auth_send_state);
    ...
    if (child_pid == 0) { /* child */
        exec_child_ex(state, pipefd_to_child, pipefd_from_child,
                      FIDO2_CHILD_PATH, state->logfile, state->extra_args,
                      false, STDIN_FILENO, STDOUT_FILENO);
        /* We should never get here */
        ret = errno;
        DEBUG(SSSDBG_CRIT_FAILURE, "BUG: Could not exec fido2 child\n");
        return ret;
...
{
    struct pam_fido2_auth_send_state *state =
                              tevent_req_data(req, struct pam_fido2_auth_send_state);

    TEVENT_REQ_RETURN_ON_ERROR(req);

    fido2_data = state->fido2_data;

    return EOK;
}

static void
pam_fido2_auth_done(int child_status,
                    struct tevent_signal *sige,
                    void *pvt)
{
    ...
    if (WIFEXITED(child_status)) {
            ...
            DEBUG(SSSDBG_OP_FAILURE,
                  FIDO2_CHILD_PATH " failed with status [%d]. Check fido2_child"
        ...
        DEBUG(SSSDBG_OP_FAILURE,
              FIDO2_CHILD_PATH " was terminated by signal [%d]. Check fido2_child"
              " logs for more information.\n",
    ...
    DEBUG(SSSDBG_TRACE_LIBS, "FIDO2 key is valid. Mark done\n");
                                ...
                                enum user_verification *_user_verification)
{
    ...
    for (c = 0; opts[c] != NULL; c++) {
        if (strcasecmp(opts[c], "user_verification") == 0) {
            ...
            DEBUG(SSSDBG_CRIT_FAILURE,
                  "Unsupported fido2 verification option [%s], " \
                  "skipping.\n", opts[c]);
        }
    }
