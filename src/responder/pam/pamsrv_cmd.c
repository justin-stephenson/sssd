...
static errno_t parse_passkey_verify_opts(TALLOC_CTX *mem_ctx,
...
void pam_passkey_get_user_done(struct tevent_req *req)
{
    ...
    domain_name = talloc_strdup(pk_data, result->domain->name);
    /* Get passkey data */
    ...
    ret = parse_passkey_verify_opts(pctx, verify_opts, &verification);
    if (ret != EOK) {
        DEBUG(SSSDBG_MINOR_FAILURE, "Unable to parse passkey verificaton options.\n");
        /* Continue anyway */
    }

    /* Use IPA require user verification setting */
    if (pk_data->user_verification != NULL) {
        if (strcmp(pk_data->user_verification, "on") == 0) {
            verification = PAM_PASSKEY_VERIFICATION_ON;
        } else if (strcmp(pk_data->user_verification, "off") == 0) {
     ...
     /* Otherwise fallback to local sssd.conf setting */
     }
    ...
    req = pam_passkey_auth_send(pctx, pctx->ev, timeout, debug_libfido2,
                                verification, pctx->pd, pk_data, domain_name);
    if (req == NULL) {
...
static void pam_forwarder_passkey_cb(struct tevent_req *req)
{
    ...
    ret = pam_passkey_auth_recv(req, preq, preq->passkey_data, &child_status);
    talloc_free(req);
    if (ret != EOK) {
