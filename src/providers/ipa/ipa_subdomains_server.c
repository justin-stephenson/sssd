...
#define TRUST_TYPE_AD 16
#define TRUST_TYPE_IPA 32
...
static errno_t ipa_server_create_trusts_step(struct tevent_req *req)
{
    ...
    DLIST_FOR_EACH(trust_iter, state->id_ctx->server_mode->trusts) {
        ...
        if (trust_iter->type == IPA_TRUST_AD) {
            sdom_a = sdap_domain_get(trust_iter->id_ctx.ad_id_ctx->sdap_id_ctx->opts,
                                     trust_iter->dom);
        } else if (trust_iter->type == IPA_TRUST_IPA) {
            sdom_a = sdap_domain_get(trust_iter->id_ctx.ipa_id_ctx->sdap_id_ctx->opts,
                                     trust_iter->dom);
        } else {
            return ENOTSUP;
        }
        ...
        DLIST_FOR_EACH(trust_i, state->id_ctx->server_mode->trusts) {
            ...
            if (trust_iter->type == IPA_TRUST_AD) {
                ...
                sdom_b = sdap_domain_get(trust_i->id_ctx.ipa_id_ctx->sdap_id_ctx->opts,
                                         sdom_a->dom);
            } else {
                return ENOTSUP;
            }
                          ...
                          struct sss_domain_info *subdom)
{
    ...
    if (iter->type == IPA_TRUST_AD) {
        sdom = sdap_domain_get(iter->id_ctx.ad_id_ctx->sdap_id_ctx->opts, subdom);
    } else if (iter->type == IPA_TRUST_IPA) {
        sdom = sdap_domain_get(iter->id_ctx.ipa_id_ctx->sdap_id_ctx->opts, subdom);
    } else {
        DEBUG(SSSDBG_CRIT_FAILURE, "Invalid trust type\n");
        return;
    }
    if (sdom == NULL) return;
    ...
    if (iter->type == IPA_TRUST_AD) {
        ...
        sdap_domain_remove(iter->id_ctx.ipa_id_ctx->sdap_id_ctx->opts, subdom);
    }
