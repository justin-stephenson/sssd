                                ...
                                struct sysdb_attrs *attrs)
{
    ...
    /* FIXME: Remove when ipa sets trust type correctly */
    if (strcasecmp(name, "ipa2.test") == 0 || (strcasecmp(name, "ipa1.test") == 0)) {
        type = 1;
    }
                          ...
                          struct sdap_handle *sh)
{
    ...
    const char *attrs[] = { IPA_CN, IPA_FLATNAME, IPA_TRUSTED_DOMAIN_SID,
                            IPA_TRUST_DIRECTION, IPA_TRUST_TYPE,
                            IPA_ADDITIONAL_SUFFIXES,
