                          ...
                          const char *prompt_interactive, const char *prompt_touch)
{
    ...
    if ((strcmp(pi->passkey_prompt_pin, "true")) == 0) {
        m[msg_idx].msg_style = PAM_PROMPT_ECHO_OFF;
    ...
    if ((strcmp(pi->passkey_prompt_pin, "true")) == 0) {
        ...
        if (resp[pin_idx].resp == NULL) {
            logger(pamh, LOG_ERR, "resp == NULL, seting SSS_AUTHTOK_TYPE_EMPTY");
            pi->pam_authtok = NULL;
            pi->pam_authtok_type = SSS_AUTHTOK_TYPE_EMPTY;
            pi->pam_authtok_size=0;
        } else {
         ...
         */
        logger(pamh, LOG_ERR, "no uv prompt, Setting SSS_AUTHTOK_TYPE_PASSKEY");
        pi->pam_authtok_type = SSS_AUTHTOK_TYPE_PASSKEY;
        pi->pam_authtok = NULL;
        pi->pam_authtok_size=0;
    }
