...
struct prompt_config {
    ...
    union {
        ...
        struct prompt_config_fido2 fido2;
...
const char *pc_get_fido2_prompt(struct prompt_config *pc)
{
    if (pc != NULL && (pc_get_type(pc) == PC_TYPE_FIDO2_INTERACTIVE
            || pc_get_type(pc) == PC_TYPE_FIDO2_TOUCH)) {
        return pc->data.fido2.prompt;
    }
    return NULL;
}
...
#define DEFAULT_FIDO2_INTERACTIVE_PROMPT _("Insert your FIDO2 device, then press ENTER.")
#define DEFAULT_FIDO2_TOUCH_PROMPT _("Please touch the device. ")
...
errno_t pc_list_add_fido2_interactive(struct prompt_config ***pc_list,
                                      const char *prompt)

{
    ...
    pc->type = PC_TYPE_FIDO2_INTERACTIVE;

    pc->data.fido2.prompt = strdup(prompt != NULL ? prompt
                                   : DEFAULT_FIDO2_INTERACTIVE_PROMPT);
...
errno_t pc_list_add_fido2_touch(struct prompt_config ***pc_list,
                                const char *prompt)

{
    ...
    pc->type = PC_TYPE_FIDO2_TOUCH;

    pc->data.fido2.prompt = strdup(prompt != NULL ? prompt
                                   : DEFAULT_FIDO2_TOUCH_PROMPT);
                                       ...
                                       uint8_t **data)
{
    ...
    for (c = 0; pc_list[c] != NULL; c++) {
        ...
        case PC_TYPE_FIDO2_TOUCH:
            l += sizeof(uint32_t);
            l += strlen(pc_list[c]->data.fido2.prompt);
            break;
        case PC_TYPE_SC_PIN:
            break;
        default:
    ...
    for (c = 0; pc_list[c] != NULL; c++) {
        ...
        case PC_TYPE_FIDO2_TOUCH:
            SAFEALIGN_SET_UINT32(&d[rp],
                                 strlen(pc_list[c]->data.fido2.prompt),
                                 &rp);
            safealign_memcpy(&d[rp], pc_list[c]->data.fido2.prompt,
                             strlen(pc_list[c]->data.fido2.prompt), &rp);
            break;
        case PC_TYPE_SC_PIN:
            break;
        default:
                              ...
                              struct prompt_config ***pc_list)
{
    ...
    for (c = 0; c < count; c++) {
        ...
        case PC_TYPE_FIDO2_INTERACTIVE:
            ...
            ret = pc_list_add_fido2_interactive(&pl, str);
            free(str);
            if (ret != EOK) {
                goto done;
            }
        ...
        case PC_TYPE_FIDO2_TOUCH:
            ...
            ret = pc_list_add_fido2_touch(&pl, str);
            free(str);
            if (ret != EOK) {
                goto done;
            }
