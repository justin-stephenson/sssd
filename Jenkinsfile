...
class Notification {
  ...
  private def getTargetURL(system) {
    if (system == "covscan") {
      return "https://github.com/SSSD/sssd/blob/master/Jenkinsfile"
    } else if (system) {
...
class Covscan extends Test {
  ...
  Covscan(pipeline, notification, repo, branch, pr_number, on_demand) {
    /* We run covscan on the agent itself, system is passed as
     * null here. */
    super(pipeline, null, notification)
  ...
  def archive() {
    this.pipeline.echo 'Internal Covscan results cannot be published to the cloud.'
  }
